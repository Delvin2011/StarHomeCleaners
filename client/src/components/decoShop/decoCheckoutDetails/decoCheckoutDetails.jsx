...
import {Popup,PopupInner,LogoContainer,Form,CloseButton,Response,Errors,Message4,Details,Right,ContentTitle,Title,PayGridSplit,PayOptions} from '../../cleaner/outdoor/outdoor-styles'; 
import {CreditCardPayment,CASpayment,PaymentOptionsContainer} from '../../cleaner/indoor/indoor-styles';
import {CartDetails, DetailHeader, BookingDetail} from './decoCheckoutDetails-styles';
...
class DecoCheckoutDetails extends React.Component {  
    constructor(props){
        super(props);   
        this.state = {
            customerName: '',
...
//https://www.telerik.com/kendo-react-ui/components/dateinputs/datetimepicker/integration-with-json/
  render() {  
    ...
    const {currentUser,total,decoCartItems} = this.props;

        return (  
            <Popup>  
                <PopupInner>                   
                        <CloseButton className = 'remove-button' style = {{"color":"black"}} onClick = {this.props.showCheckoutDetails} >&#10005;</CloseButton>  
                        ...
                        <Title> Enter Contact Details
                                ...
                                <Details> 
                                    ...
                                    <Right >
                                        ...
                                        <DetailHeader > Cart Items </DetailHeader>                                   
                                        <CartDetails>
                                        ...
                                        </CartDetails>
                                            {
                                                ...
                                                <CartDetails key={key}>
                                                    <BookingDetail>{decoCartItems[key].name}</BookingDetail>
                                                    <BookingDetail style = {{textAlign : "center"}}>{decoCartItems[key].quantity}</BookingDetail>
                                                </CartDetails>
                                                )                        
                                            } 
                                        </div>
                                        <DetailHeader> Total Costs: R{total.toFixed(2)}</DetailHeader>                                    
                                    </Right>  
                                    </Details> 

                                    <Title> PAYMENT METHODS </Title>
                                    <PaymentOptionsContainer>
                                        <PayGridSplit><Fab><CASpayment onClick = {this.CashAfterServicePayment.bind(this)} IndoorCashPayment = {this.state.IndoorCashPayment}/></Fab><PayOptions>COD (Cash On Delivery)</PayOptions></PayGridSplit>
                                    ...
                                    {   this.state.IndoorCashPayment ?
                                            ...
                                            : response === 500 || response === 404 ?
                                                <div>
                                                    ...
                                                    <p style = {{"textAlign" : "center"}}><CustomButton type = 'submit'  style = {{"margin-top" : "10px", "background": "#e91e63"}} size="sm">RESUBMIT</CustomButton></p> 
                                                </div>
...
export default connect(mapStateToProps)(DecoCheckoutDetails);
