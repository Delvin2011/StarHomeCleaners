    ...
    switch (action.type){
        ...
        case DecoCartActionTypes.ADD_DECOITEM:
            ...
            case DecoCartActionTypes.REMOVE_DECOITEM:
                return{
                    ...state,
                    decoCartItems: removeDecoItemFromCart(state.decoCartItems, action.payload) //we are trying to keep the item with the id of the item that we don't need to clear out, and we remain with new array
                }     

        case DecoCartActionTypes.CLEAR_DECOITEM_FROM_CART:
            return{
                ...state,
                decoCartItems: state.decoCartItems.filter(decoCartItem => decoCartItem.id !== action.payload.id) //we are trying to keep the item with the id of the item that we don't need to clear out, and we remain with new array
            }   
